export async function fetchRepo(owner: string, perPage: number = 5): Promise<any | null> {
	try {
		...
		if (!reposResponse.ok) {
			throw new Error(
				`Error fetching repositories: ${reposResponse.status}`
			);
		}
...
async function fetchAllRepoLanguages(
	...
	try {
		...
		const languagesPromises = repos.map(
		...
		console.error(
			'Failed to fetch languages for all repositories:',
			error
		);
